...
constexpr long timer1Frequency{ 1000L };                                    // timer frequency (1000 Hz by default)

// brightness levels (steps up or down) and minimum brightness level for each individual LED
constexpr uint16_t brightness_bits[3]{ 4, 5, 9 };                           // number of bits used (4 to 10 bits are good choices: 16 to 1024 brightness levels)
constexpr uint16_t lowestBrightnessMinimalFlicker[3]{ 0, 4, 20 };           // brightness levels lower than these would produce flicker
...
And, with the correct lowest brightness settings applied, without flicker !
...
void loop()
{
    ...
    changeBrightnessNow[0] = (currentTime >= lastTime[0] + 20);                 // ... LED 1: every 20 ms: asynchronous with LED refresh periods
    changeBrightnessNow[1] = (currentCount[1] > lastCount[1] + 1);              // ... LED 2: every 2 LED refresh periods (synchronous)
    ...
    for (int index = 0; index < 3; index++) {                                   // for each LED
        if (changeBrightnessNow[index]) {                                       // change brightness ?
            ...
            if (index == 2) { Serial.println(br); }////
            cli(); brightness[index] = br; sei();                               // pass brightness to ISR (who will pick it up at the start of a new LED refresh period)
        }
    }
}
...
/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    (This is the same algorithm used by hardware to draw a line with a slope (less than 1) expressed as a ratio of two integer numbers. In this example: slope = 7/31)
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
SIGNAL(TIMER1_OVF_vect) {
    ...
    for (int index = 0; index < 3; index++) {                                                   // for the red, green, blue LED
        // start of a new LED refresh period ? Then LOAD the desired LED brightness (set by the main program during the last LED refresh period) 
